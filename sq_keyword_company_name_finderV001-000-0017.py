...
# Version file: V001-000-0015 #    Update with next iteration

# Version change notes:
# 
#         I'm copying V001-000-0015 instead of 16. I'm goign to use this output to write a csv file.
#
#         --note: load the name column along with the company column from the company_file
#              Modify the process_search_queries function to store both the unmatched search queries and their corresponding names from the company_file
#               store unmatched search queries along with their corresponding names from the company_file.
#               compare the unmatched search queries with the values in the name column of the company_file to identify matches. If a match is found, we append it to the matches list.
#               call the output_highest_match function to process the extracted data accordingly.
#               
#               
...
def output_highest_match(matches):
    ...
    csv_file = 'data/sq_keyword_company_name_finder_output.csv'
    with open(csv_file, 'w', newline='', encoding= 'utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['search_query', 'company_name', 'ticker', 'filtered_sorted_data'])
        for match in matches:
            ...
            writer.writerow([search_query, company_name, ticker, filtered_and_sorted_data])
