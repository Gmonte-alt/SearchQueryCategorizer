...
# Version file: V001-000-0010 #    Update with next iteration
...
#
#         --note: FIND ERRORS I need to find the unmatched search queries. This version edit allows us to do this.
#               introduced a new list called unmatched_queries to store the search queries that were not matched.
#               Inside the loop that iterates over the search queries, if no matches are found for a particular query, it gets added to the unmatched_queries list.
#               After processing all search queries, the output_highest_match function is called as before.
#               Finally, if there are any unmatched queries, they are printed out. Found error around two word company names. Next version to resolve.
                    ...
                    for i, search_word in enumerate(search_query_words):
                        for j, company_word in enumerate(company_words):
                            if search_word.lower() == company_word.lower():
                                ...
                                if search_word == 'microsoft': # 'growth fund of america': # 'realty income'
