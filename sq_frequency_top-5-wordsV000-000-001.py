...
def get_top_triplets(word_ff_purchases, top_cooccurring_words, top_n=1):
    ...
    for word, top_words in top_cooccurring_words.items():
        for co_word, _ in top_words:
            ...
            for third_word, ff_purchases in third_level_words:
                if third_word != word:
                    top_triplets[word].append((co_word, third_word, ff_purchases))
...
def write_top_triplets(output_file, top_triplets):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        writer.writerow(["Word", "Co-occurring Word 1", "Co-occurring Word 2", "Total ff_purchases"])
        for word, triplets in top_triplets.items():
            for co_word, third_word, ff_purchases in triplets:
                writer.writerow([word, co_word, third_word, round(ff_purchases, 2)])
...
if __name__ == "__main__":
    ...
    output_file = "output/top_triplets_by_purchases.csv"  # Output CSV file
    ...
    # Get the top triplets based on ff_purchases
    top_triplets = get_top_triplets(word_ff_purchases, top_cooccurring_words, top_n=1)

    # Write the top triplets to the output file
    write_top_triplets(output_file, top_triplets)

    print(f"Top triplets by purchases have been saved to {output_file}")
