...
def main():
    ...
    for query in search_queries:
        ...
        search_query_results = pd.concat([search_query_results, pd.DataFrame({'Search Query': [query], 'Matching Requirement Met': [matching_requirement_met], 'Status': ['open'], 'qualified_target': [False], 'targetable keyword': [''], 'exact_match': [False], 'named_entity': [False], 'stand-alone': [False]})], ignore_index=True)
