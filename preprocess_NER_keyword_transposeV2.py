...
def process_csv(csv_file):
    ...
    for index, row in df.iterrows():
        ...
        for idx, word in enumerate(words):
            ...
            # Assign appropriate tag based on position and count
            if word != '.':
                if first_word:
                    if len(words) == 1 or (idx == len(words) - 1 and len(word) == 1):  # Single word or last word before a period and of length 1
                        rows.append([word, 'U-' + tag])
                    elif idx == len(words) - 1:  # Last word before a period
                        rows.append([word, 'L-' + tag])
                    else:  # First word in a sequence
                        rows.append([word, 'B-' + tag])
                    first_word = False
                elif idx == len(words) - 1:  # Last word before a period
                    rows.append([word, 'L-' + tag])
                else:  # Subsequent words in a sequence
