...
# Version file: V001-000-0012 #    Update with next iteration
...
#
#         --note: Need to filter-out words like "inc" prior to the "check_context" function.
#               defined a set called common_words containing the words you want to filter out.
#               Within the loop that iterates over the company names, I filter out the common words from each company name before splitting it into words.
#               The filtered_company_name is then used for matching instead of the original company_name.
#               This ensures that the matching process ignores the common words specified in the common_words set.
    ...
    with open(search_query_file, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            for company_name in company_names:
                # Filter out common words from company name
                filtered_company_name = ' '.join(word for word in company_name.split() if word.lower() not in common_words)
                company_words = filtered_company_name.split()
                ...
                if len(search_query_words) == 1 or len(company_words) == 1:
                    if search_query.lower() == company_name.lower():
                        ...
                        #matches.append((search_query, company_name, match_count, "Single word match", "Single word match"))
                        matches_found = True
                else:
                    for i, search_word in enumerate(search_query_words):
                        for j, company_word in enumerate(company_words):
                            if search_word.lower() == company_word.lower():
                                ...
                                if matched_words_count > 0 and search_word == 'microsoft': # 'growth fund of america': # 'realty income'
                ...
                #if matches_found:
                                    matches.append((search_query, company_name, match_count, position, company_word_position))
