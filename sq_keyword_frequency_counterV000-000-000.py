...
def count_word_frequency(query_file):
    ...
    return word_frequency, impressions_total, clicks_total, conversions_total, spend_total

def write_word_frequency(word_frequency, impressions_total, clicks_total, conversions_total, spend_total, output_file):
    """Write word-frequency pairs and total metrics to an output CSV file."""
    sorted_word_frequency = sorted(word_frequency.items(), key=lambda x: x[1], reverse=True)
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Word", "Frequency", "Total Impressions", "Total Clicks", "Total Conversions", "Total Spend", "Total Cost Per Click", "Total Cost Per Conversion", "Total Conversion Rate", "Total Click Thru Rate"])
        for word, frequency in sorted_word_frequency:
            ...
            writer.writerow([word, frequency, total_impressions, total_clicks, total_conversions, total_spend, total_cost_per_click, total_cost_per_conversion, total_conversion_rate, total_click_thru_rate])

if __name__ == "__main__":
    query_file = "data/stock picks-KeywordStats2024-04-11at08_49_40-download.csv"  # data/2023-2024ConvertingSearchTermsReport-SearchTerm_dedupped_metrics.csv # Path to the CSV file containing search queries and metrics
    output_file = "output/search_query_word_frequency_kwplanner_nvda.csv"  # "data/search_query_word_frequency.csv" # Output CSV file
    word_frequency, impressions_total, clicks_total, conversions_total, spend_total = count_word_frequency(query_file)
    write_word_frequency(word_frequency, impressions_total, clicks_total, conversions_total, spend_total, output_file)