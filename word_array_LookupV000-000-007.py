...
# Output file:
# Notes: This next version builds on V000-000-006;
#        This code identifies the single word search queries from multi-word search queries
...
def process_search_query(query, lookup_results):
    ...
    is_single_word_query = len(query.split()) == 1
    ...
    for idx, result in enumerate(lookup_results):
        if any(value == False for value in result.values()):
            ...
            for word, (match_type, is_standalone, is_named_entity) in result.items():
                # For single word query, check exact match and named_entity
                if is_single_word_query:
                    if match_type == 'Ticker' and word.lower() == query.lower():
                        ...
                        stand_alone = True
                        break
                # elif is_single_word_query == False:
                #     print(word, result, lookup_results)
                else:  # For multi-word query, consider named_entity for any matching word
                    if is_named_entity:
                        named_entity = True
                    if is_standalone:
                        stand_alone = True
